...
def train(input_tensor, target_tensor, mask_input, mask_target, encoder, decoder, bridge, encoder_optimizer, decoder_optimizer, bridge_optimizer,device, criterion, max_length,batch_size=32,bidirectional=False,teacher_forcing=True):
    ...
    for step_idx in range(batch_size):
        ...
        encoder_outputs = torch.zeros((batch_size, max_length, encoder.hidden_sizes))#CODE_BLANK_2
        ...
        for ei in range(input_length):
            ...
            encoder_output, encoder_hidden = encoder(input_tensor[ei], encoder_hidden)#Code_BLANK_3
            
            #Store encoder output at each word at each batch in the encoder outputs tensor
            encoder_outputs[step_idx, ei, :] = EncoderRNN(encoder.hidden_size,encoder_output[0, 0], )
    ...
    decoder_hiddens = encoder_hiddens_last[-1]#CODE_BLANK_6
    ...
    if teacher_forcing:
        ...
        for step_idx in range(batch_size):
            ...
            for di in range(target_length):
                ...
                decoder_output, decoder_hidden = decoder(decoder_input, decoder_hidden, encoder_outputs)#CODE_BLANK_7
        ...
        loss = loss/batch_size#Code_BLANK_8
...
def trainIters(trainloader,encoder, decoder, bridge,device,bidirectional=False,teacher_forcing=True,num_epochs=600,batch_size=32,max_length=20, print_every=1000, plot_every=100, learning_rate=0.1):
    ...
    for i in range(num_epochs):
        ...
        for iteration,_ in enumerate(trainloader, 1):
            ...
            training_pair = trainloader[iteration]#CODE_BLANK_2
            ...
            loss = train(input_tensor, target_tensor, mask_input, mask_target, encoder, decoder, bridge, encoder_optimizer, decoder_optimizer, bridge_optimizer, device, criterion, max_length, batch_size#Enter your arguments here, Dont change the below arguments
